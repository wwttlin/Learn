...
# 3. 停止現有服務
print_info "停止現有服務..."
sudo pkill -9 -f "node.*server/index.js" 2>/dev/null || true
...
if ps -p $SERVER_PID > /dev/null; then
    ...
    # 測試 API 連接
    sleep 2
    if curl -s --max-time 10 "http://localhost:5000/api/students" > /dev/null; then
        print_status "API 連接測試成功"
    else
        print_warning "API 連接測試失敗，但服務可能仍在啟動中"
    fi
else
    print_error "後端服務啟動失敗"
    print_info "檢查日誌: cat server.log"
    exit 1
fi
...
if [ "$USE_PM2" = true ]; then
    # 使用 PM2 啟動
    pm2 start server/index.js --name "tutoring-backend" --max-memory-restart 500M
    pm2 save
    pm2 startup
...
# 12. 顯示部署結果
EXTERNAL_IP=$(curl -s --max-time 5 ifconfig.me 2>/dev/null || echo "your-server-ip")
